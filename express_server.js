...
const { getUserByEmail, generateRandomString, urlsForUser } = require("./helpers"); //imports helper functions
...
const urlDatabase = {}; //empty object to store urls in thier short and long forms
...
app.delete("/urls/:id", (req, res) => {// add route to delete urls and redirect to main page // uses HTTP delete method using method override
...
app.put("/urls/:id", (req, res) => { //uses HHTP put method using method override
...
app.post("/login", (req, res) => { //route to login
...
app.post("/logout", (req, res) => { //route to logout when user logs out
  req.session = null;//clears the coookie when loggin out
  res.redirect("/urls");
});
...
app.post("/register", (req, res) => { //route to add user info to blobal users object
  ...
  const hashedPassword = bcrypt.hashSync(userPassword, 10); // hashes the password before saving it to the users object
